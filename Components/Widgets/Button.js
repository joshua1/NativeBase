...
export default class Button extends NativeBaseComponent {
  ...
  prepareRootProps() {
    ...
    const addedProps = _.merge(this.getInitialStyle().button, type);
  ...
  getIconProps(icon) {
    const defaultStyle = {
      color:
                ...
                (this.props.transparent) ? this.getContextForegroundColor() :
                this.getTheme().inverseTextColor,
      marginBottom: ((this.props.vertical) && (this.getPlatform() == 'android')) ? 2 : undefined,
  ...
  renderChildren() {
    if (typeof this.props.children == 'string') {
      return <Text style={this.getTextStyle()}>{(this.getPlatform() === 'ios' || !this.props.capitalize) ? this.props.children : this.props.children.toUpperCase()}</Text>;
      ...
      if (this.props.iconRight) {
        if (childrenArray[0].type == undefined) {
          newChildren.push(<Text key="label" style={this.getTextStyle()}>{(this.getPlatform() === 'ios' || !this.props.capitalize) ? childrenArray[0] : childrenArray[0].toUpperCase()}</Text>);
        } else {
          newChildren.push(<Text key="label" style={this.getTextStyle()}>{(this.getPlatform() === 'ios' || !this.props.capitalize) ? childrenArray[0].props.children : childrenArray[0].props.children.toUpperCase()}</Text>);
        }
        ...
        return <Text style={this.getTextStyle()}>{(this.getPlatform() === 'ios') ? this.props.children : this.props.children.toUpperCase()}</Text>;
      }
