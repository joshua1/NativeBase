  ...
  getInitialStyle() {
    return {
      ...
      btnTextStyle: {
        ...
        fontSize: (this.getPlatform() == 'ios') ? 12.5 : 10,
        lineHeight: 13,
      },
      btnActiveTextStyle: {
        color: this.getTheme().tabBarActiveTextColor,
        fontSize: (this.getPlatform() == 'ios') ? 12.5 : 11,
        lineHeight: 13,
      },
      btnStringTextStyle: {
        color: this.getTheme().tabBarTextColor,
        fontSize: (this.getPlatform() == 'ios') ? 14 : 11,
        lineHeight: 16,
      },
      btnStringActiveTextStyle: {
        color: this.getTheme().tabBarActiveTextColor,
        fontSize: (this.getPlatform() == 'ios') ? 14 : 12,
        lineHeight: 16,
      },
      btnStyle: {
        alignSelf: 'center',
        ...
        borderRadius: (this.getPlatform() == 'ios') ? 7 : 0,
      },
    };
  }
  ...
  renderFooter() {
    ...
    { childrenArray.map((child, i) => {
      if (typeof child.props.children === 'string') {
        ...
        let iconElement = [];
        iconElement = _.remove(child.props.children, (item) => {
          if (item.type == IconNB) {
            return true;
          }
        });
        let badgeElement = [];
        badgeElement = _.remove(child.props.children, (item) => {
          if (item.type == Badge) {
            return true;
          }
        });
